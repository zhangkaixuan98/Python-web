...
def aim():
    db = MySQL()
    sql = "select user_aim, count(*) from blog_user group by user_aim"
    aim = db.ExecQuery(sql)
    aim_list = []
    for row in aim:
        if not row[0] == '':
            aim_dict = {'aim': row[0], 'number': row[1]}
            aim_list.append(aim_dict)
    jsonDate = json.dumps(aim_list, ensure_ascii=False)
    print(jsonDate)
